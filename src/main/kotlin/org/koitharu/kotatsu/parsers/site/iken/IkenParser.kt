...
internal abstract class IkenParser(
...
) : PagedMangaParser(context, source, pageSize), Interceptor {
    ...
    override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
        ...
        return parseMangaList(webClient.httpGet(url).parseJsonSafe())
    }
    ...
    override suspend fun getDetails(manga: Manga): Manga {
        ...
        val json = webClient.httpGet(url).parseJsonSafe().getJSONObject("post")
    ...
    override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
        ...
        val imagesJson = try {
            doc.getNextJson("images")
        } catch (e: Exception) {
            doc.getNextJson("API_Response")
        }
    ...
    private fun parseImagesJson(json: String): List<String> {
        return if (json.trim().startsWith("[")) {
            ...
            // Fallback for "API_Response" or other object structures
            val obj = JSONObject(json)
            val chapter = obj.optJSONObject("chapter") ?: obj
            val images = chapter.optJSONArray("images") ?: throw Exception("No images found in JSON")
    ...
    private fun Response.parseJsonSafe(): JSONObject {
        if (header("Content-Type")?.contains("json") == false) {
            val url = request.url.toString()
            close()
            throw ParseException("Cloudflare protection or server error", url)
        }
        return parseJson()
    }
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (response.code == 403 && useAPI && request.url.host == defaultDomain) {
            if (CloudFlareHelper.checkResponseForProtection(response) != CloudFlareHelper.PROTECTION_NOT_DETECTED) {
                response.close()
                // Fetch main page to get cookies
                // Use synchronous call since intercept is not suspend
                val mainPageRequest = Request.Builder()
                    .url("https://$domain")
                    .build()
                
                runCatching {
                     context.httpClient.newCall(mainPageRequest).execute().close()
                }
